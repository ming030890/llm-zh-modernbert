...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The pooling to use for the classifier head. If not passed, the default pooling will be used.",
    )
...
def main():
    ...
    if args.task_name is not None:
        ...
        raw_datasets = load_dataset(
            "shunk031/JGLUE", args.task_name, trust_remote_code=True
        )
    else:
    ...
    # Labels
    if args.task_name is not None:
        is_regression = args.task_name == "stsb" or args.task_name == "JSTS"
    ...
    if args.classifier_pooling is not None:
        config.classifier_pooling = args.classifier_pooling
    ...
    if args.task_name is not None:
        if args.task_name == "JSTS":
            metric = evaluate.load("glue", "stsb")
        elif args.task_name == "JNLI":
            metric = evaluate.load("glue", "mnli")
        elif args.task_name == "JCoLA":
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            predictions = predictions.reshape(references.shape)
            metric.add_batch(
                predictions=predictions,
                references=references,
            )
    ...
    if args.output_dir is not None:
        ...
        with open(
            os.path.join(
                ...
                args.task_name,
                args.model_name_or_path,
                f"all_results_e{args.num_train_epochs}_lr{args.learning_rate}.json",
            ),
            "w",
        ) as f:
